...
export default function Pehsai() {
  ...
  const [centerRadius, setCenterRadius] = useState(112);
  const baseTurnsRef = useRef(0);
  const spinTimerRef = useRef(null);

  const segAngle = useMemo(() => (names.length > 0 ? 360 / names.length : 0), [names]);
  const centerClipId = useMemo(() => `center-clip-${Math.random().toString(36).slice(2)}`, []);
  ...
  return (
    <div className="pehsai-layout">
                  ...
                  {centerImg && (
                    <>
                      <image
                        ...
                        clipPath={`url(#${centerClipId})`}
                      />
                      <circle cx={cx} cy={cy} r={centerRadius} fill="none" stroke="#ffffff" strokeWidth="4" />
                    </>
                  )}
                </g>
              </svg>
      ...
      <aside className="side-panel">
        ...
        <div className="center-radius-control">
          <label htmlFor="center-radius-slider">Center Image Size</label>
          <input
            id="center-radius-slider"
            type="range"
            min="112"
            max="120"
            step="1"
            value={centerRadius}
            onChange={(e) =>
              setCenterRadius(Math.max(112, Math.min(120, Math.floor(Number(e.target.value) || 112))))
            }
          />
          <small>{centerRadius}px radius (112–120px)</small>
        </div>
