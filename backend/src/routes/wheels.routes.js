...
router.post('/:id/spin', async (req, res, next) => {
  try {
    ...
    // Percentage prizes need a valid bill amount to compute rewards
    ...
    if (eligible.length === 0) {
      return res.status(409).json({ message: 'No offers available at this moment.' });
    }
    ...
    if (seg.prizeType === 'percentage') {
      ...
      const spent = Number(session.amountSpent);
    ...
    const spent = Number(session.amountSpent);
    const payable = (percentageValue !== null && Number.isFinite(spent) && Number.isFinite(computedReward))
      ? Math.max(0, Number((spent - computedReward).toFixed(2)))
      : null;
    const percentSummary = (percentageValue !== null && payable !== null)
      ? `Paid ₹${payable} instead of ₹${spent} - ${percentageValue}% off applied`
      : (seg.amount || '');

    session.winner = seg.text;
    session.prizeType = seg.prizeType || 'other';
    session.prizeAmount = percentSummary;
    session.percentageValue = percentageValue;
    session.computedReward = computedReward;
    session.userId = userId;
    session.outTime = new Date();
    
    // For percentage discounts, update amountSpent to the final payable amount
    ...
    // Create analytics record for tracking rewards
    // For percentage discounts, store the final payable amount as amountSpent
    const finalAmountSpent = seg.prizeType === 'percentage' && payable !== null 
      ? String(payable) 
      : session.amountSpent;
    ...
    const analyticsRecord = await Analytics.create({
      ...
      originalPrice: seg.prizeType === 'percentage' ? Number(session.amountSpent) : null,
