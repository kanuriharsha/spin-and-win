...
- `prizeType`: String (enum: 'cash', 'loyalty', 'percentage', 'other')
- `amount`: String (e.g., "₹500", "30 Loyalty Points", "20% OFF")

**SpinResults Collection** - Each result now stores:
- `winner`: Prize name
- `prizeType`: Type of prize won (now includes 'percentage')
- `prizeAmount`: Amount/details of prize
- `userId`: Link to user (for loyalty points)
- `percentageValue` / `computedReward`: Captured for percentage rewards
...
- Sends complete prize details (including percentageValue/computedReward) to backend
